...
.. note::
	...
	Clever readers will notice that each training epoch only used 320 training
	...
	running this example on my tiny ultrabook on an Intel Core m7 CPU. As you'll
	see in `Under the Hood`_, I truncate the training process to only train
	on 10 batches of 32 samples each, just to make the training loop finish in
	a reasonable amount of time. It's not cheating: you still get 90% accuracy!
	But if you have awesome hardware, or just want to see how good your accuracy
	can get, then by all means read on and we'll show you how to modify that.
...
.. literalinclude:: ../../examples/mnist.yml
   :language: yaml
   :caption: mnist.yml
...
The first block of the file is the ``train`` section, which describes the data
that your model uses to train. Here, we just point it to the MNIST data online,
denoted by the ``images`` and ``labels`` tags. Cool, data!

The only other part you need is the model itself. What's in a Kur model? Just a
nice, gentle list of things you want your deep learning model to do. Let's break
it down:
